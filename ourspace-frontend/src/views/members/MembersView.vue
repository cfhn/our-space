...
import {
  ...
  type MemberServiceListMembersResponse,
...
const withCustomType = createFeature(() => ({
  ...
  typeRenderer: {
    tags: DataGridFeatures.createTypeRenderer<object, MemberEntry>({
      cell: {
        component: ({ modelValue }) => {
          const tags = Array.isArray(modelValue)
            ? modelValue
            : modelValue
              ? [modelValue.toString()]
              : []
    ...
    actions: DataGridFeatures.createTypeRenderer<object, MemberEntry>({
      cell: {
        tdAttributes: {
...
watch(
  [currentPageToken, searchValue, reload],
  async () => {
...
<template>
  <OnyxPageLayout>
    ...
    <OnyxModal label="Delete member" :open="deleteMemberDialogOpenFor !== undefined">
      <template #default>
        <div class="modal">
          <p>Do you really want to delete the member? If yes, click Delete.</p>
          <p>Alternatively, you can end the membership.</p>
        </div>
      </template>
      <template #footer>
        <OnyxBottomBar>
          <OnyxButton
            ...
            color="neutral"
            mode="plain"
            @click="deleteMemberDialogOpenFor = undefined"
          />
          <OnyxButton
            label="End membership"
            color="primary"
            mode="plain"
            @click="endMembership(deleteMemberDialogOpenFor ?? '')"
          />
          <OnyxButton
            label="Delete"
            color="danger"
            mode="plain"
            @click="deleteMember(deleteMemberDialogOpenFor ?? '')"
          />
        </OnyxBottomBar>
      </template>
    </OnyxModal>
