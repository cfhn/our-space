...
const rfidValue = computed(() =>
  [...atob(props.card.rfid_value)]
...
<template>
  <OnyxCard
    :class="{ 'makerspace-card': true, 'makerspace-card-invalid': !isValid }"
    :style="{ 'background-image': `url('${mseCard}')` }"
    @click="openCard(card.id)"
  >
    <OnyxAvatar :full-name="member.name" size="48px" />
    <div class="name">{{ member.name }}</div>
    <div class="validity">{{ validFrom }} - {{ validTo }}</div>
